...
{
    printf("(Max 3072 characters : hit ESC-ENTER to end)\n"); // Prompt user for maximum input size.

    char itype; // Type of inode ('D' for directory, 'F' for file).
    int blocks[3]; // Array to store block indices.
    _directory_entry _directory_entries[4]; // Array to hold directory entries.

    int temp_directory_newblock = -1; // Temporary storage for new directory block.
    int temp_file_newblocks[3] = {-1, -1, -1}; // Temporary storage for new file blocks.
    int i, j; // Loop counters.

    int empty_dblock = -1, empty_dentry = -1; // Variables to track empty directory block and entry.
    int empty_ientry; // Index for empty inode entry.
    ...
    // Check if there are free inode entries.
    if (free_inode_entries == 0)
    {
        printf("Error: Inode table is full.\n");
        return;
    ...
    // Read inode entry for current directory.
    // In SFS, an inode can point to three blocks at the most.
    itype = _inode_table[CD_INODE_ENTRY].TT[0]; // Fetch type of inode.
    blocks[0] = stoi(_inode_table[CD_INODE_ENTRY].XX, 2); // Fetch block indices.
    blocks[1] = stoi(_inode_table[CD_INODE_ENTRY].YY, 2);
    blocks[2] = stoi(_inode_table[CD_INODE_ENTRY].ZZ, 2);

    // If it's a file, abort operation.
    if (itype == 'F')
    {
        printf("Fatal Error! Aborting.\n");
        exit(1);
    ...
    for (i = 0; i < 3; i++)
    {
        ...
        { // If block is empty.
            if (empty_dblock == -1)
                empty_dblock = i; // Store index of empty block.
            continue;
        }

        readSFS(blocks[i], (char *)_directory_entries); // Read directory entries.
        ...
        for (j = 0; j < 4; j++)
        {
            ...
            { // Unused entry found.
                if (empty_dentry == -1)
                {
                    empty_dentry = j; // Store index of empty entry.
                    empty_dblock = i; // Store index of empty block.
                }
                continue;
            }
    ...
    // If no empty directory entry found and all three blocks are in use, exit.
    if (empty_dentry == -1 && empty_dblock == -1)
    {
        printf("Error: Maximum directory entries reached.\n");
        return;
    ...
    { // Otherwise
        if (empty_dentry == -1)
        { // No empty entry found but not all blocks are used.
            empty_dentry = 0;

            // Get a new block using the block bitmap.
            if ((blocks[empty_dblock] = getBlock()) == -1)
            {
                printf("Error: Disk is full.\n");
                return;
            ...
            // Update the inode entry of current directory to reflect the new block usage.
            switch (empty_dblock)
            {
            case 0:
                itos(_inode_table[CD_INODE_ENTRY].XX, blocks[empty_dblock], 2);
        ...
        char input_char = 0;
        char input_arr[3][1024]; // Array to store file content.
        int flag_t = 0; // Flag to mark end of input.
        int input_count = 0; // Count of characters inputted.
        ...
        for (int i = 0; i < 3; ++i)
        {
            ...
            // If ESC is pressed, stop taking input.
            if (flag_t)
            {
                break;
            }
        ...
        // Check for overflow in input.
        char ttt = 0;
        scanf("%c", &ttt);
        if (!flag_t && ttt != 27)
        {
            while (1) // Dump the extra input.
            {
                scanf("%c", &ttt);
                if (ttt == 27)
                {
        ...
        // Create the inode entry for the new file.
        strncpy(_inode_table[empty_ientry].TT, "FI", 2); // File type.
        strncpy(_inode_table[empty_ientry].XX, "00", 2); // No blocks assigned initially.
        strncpy(_inode_table[empty_ientry].YY, "00", 2);
        strncpy(_inode_table[empty_ientry].ZZ, "00", 2);
        ...
        for (int i = 0; i < 3; ++i)
        {
            if (input_arr[i][0]) // If block is used.
            {
                if ((temp_file_newblocks[i] = getBlock()) == -1)
                {
                    // Return all new blocks and inode used in this function.
                    printf("Error: Disk is full.\n");
                ...
                else
                {
                    ...
                    switch (i)
                    { // Update the inode entry of current dir to reflect the new block usage.
                    case 0:
        ...
        // Read block of current directory where info on this new directory will be written.
        readSFS(blocks[empty_dblock], (char *)_directory_entries);
        _directory_entries[empty_dentry].F = '1'; // Mark directory entry as used.
        strncpy(_directory_entries[empty_dentry].fname, fname, 252); // Copy filename.
        itos(_directory_entries[empty_dentry].MMM, empty_ientry, 3); // Store index of inode.
        writeSFS(blocks[empty_dblock], (char *)_directory_entries); // Write directory block back to disk.

        writeSFS(BLOCK_INODE_TABLE, (char *)_inode_table); // Write the inode table back to the disk.
        printf("%d bytes saved.\n", input_count); // Print number of bytes saved.
    }
}
...
{
    char itype; // Type of the current inode (file or directory)
    int blocks[3]; // Array to store block pointers of the current inode
    _directory_entry _directory_entries[4]; // Array to store directory entries

    int i, j; // Loop counters
    int e_inode; // Inode of a directory entry

    // read inode entry for current directory
    // in SFS, an inode can point to three blocks at the most
    itype = _inode_table[current_inode].TT[0]; // Retrieve type of inode
    blocks[0] = stoi(_inode_table[current_inode].XX, 2); // Convert block pointers from binary and store
    blocks[1] = stoi(_inode_table[current_inode].YY, 2);
    blocks[2] = stoi(_inode_table[current_inode].ZZ, 2);

    // its a directory; so the following should never happen
    if (itype == 'F') // Check if inode represents a file
    {
        for (int ii = 0; ii < 3; ++ii)
            returnBlock(blocks[ii]); // Return blocks pointed by the inode
        returnInode(current_inode); // Return the inode itself
        return;
    }

    // lets traverse the directory entries in all three blocks
    for (i = 0; i < 3; i++) // Iterate over the blocks pointed by the inode
    {
        if (blocks[i] == 0)
            continue; // 0 means pointing at nothing, so move to the next block

        readSFS(blocks[i], (char *)_directory_entries); // Read directory entries from the block; casting to char pointer

        // so, we got four possible directory entries now
        for (j = 0; j < 4; j++) // Iterate over directory entries in the block
        {
            if (_directory_entries[j].F == '0')
                continue; // Unused entry, move to the next entry

            e_inode = stoi(_directory_entries[j].MMM, 3); // Retrieve the inode from the directory entry

            remove2(e_inode); // Recursively remove the directory entry
        }
        returnBlock(blocks[i]); // Return the block
    }
    returnInode(current_inode); // Return the current inode
}
